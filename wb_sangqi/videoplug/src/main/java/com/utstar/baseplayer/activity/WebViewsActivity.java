...
public class WebViewsActivity extends FragmentActivity {
  ...
  private void setUrl() {
    ...
    ToastUtil.getInstace().showToastByString(this, "session:" + session, 20);
    setCookie(url);
    //webView.loadUrl(url);
    webView.loadUrl("http://172.18.104.59:8080/");
    //        webView.loadUrl("http://192.168.4.182:8400/esUT/Default.aspx");
    //        webView.loadUrl("http://www.sogou.com/web?ie=UTF-8&query=android+webkit%E5%BC%80%E5%8F%91");
  ...
  private void setCookie(String url) {
    String session = getSession();
    session="sssss";
    showToask("开始设置cookie，cookie为：JSESSIONID=" + session);
  ...
  private void showToask(final String msg) {
    runOnUiThread(new Runnable() {
      @Override public void run() {
        ToastUtil.getInstace().showToastByString(WebViewsActivity.this, msg, 20);
      }
    });
  }
